...
export default function AdminPage() {
  ...
  // Yeni Etkinlik State'leri (Fiyat alanı geri eklendi)
  ...
  const [editEvtPrice, setEditEvtPrice] = useState('0')
  ...
  const handleUpdateEvent = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from('events').update({
      ...
      image_url: editEditImage || editingEvent.image_url
      ...
      <div className="mx-auto max-w-7xl space-y-12 mb-16">
        ...
        {isSuperAdmin && (
          <div className="rounded-3xl border border-primary/30 bg-zinc-950 p-6 sm:p-8 shadow-2xl space-y-6">
            <h2 className="text-base font-bold flex items-center gap-2 text-primary"><Calendar className="h-5 w-5" /><span>Yeni Topluluk Etkinliği / Buluşması Oluştur</span></h2>
            <form onSubmit={handleAddEvent} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <input type="text" placeholder="Konum / Detay Notu" value={evtLocation} onChange={(e) => setEvtLocation(e.target.value)} className="rounded-xl border border-white/10 bg-black px-4 py-3 text-xs text-white" />
              ...
              <input type="number" placeholder="Ücret (0 = Ücretsiz)" required value={evtPrice} onChange={(e) => setEvtPrice(e.target.value)} className="rounded-xl border border-white/10 bg-black px-4 py-3 text-xs text-white" />
              ...
              <textarea rows={1} placeholder="Açıklama (Detaylar için Instagram yönlendirmesi ekleyebilirsin)" value={evtDesc} onChange={(e) => setEvtDesc(e.target.value)} className="sm:col-span-2 lg:col-span-2 rounded-xl border border-white/10 bg-black px-4 py-3 text-xs text-white" />
        ...
        {/* ETKİNLİK BAŞVURULARI & ÖN KAYIT YÖNETİMİ */}
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            <h2 className="text-base font-bold flex items-center gap-2"><Users className="h-4 w-4 text-primary" /><span>Ön Kayıt / Katılım Talepleri ({filteredRegistrations.length})</span></h2>
