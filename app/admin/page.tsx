              ...
              <div className="border border-red-500/30 bg-red-500/10 p-4 rounded-2xl space-y-3">
                ...
                {selectedMember.ban_reason ? (
                  <div className="space-y-2">
                    <p className="text-red-300 text-[11px]">Uzaklaştırma sebebi: <strong className="text-white">{selectedMember.ban_reason}</strong></p>
                    <button type="button" onClick={async () => {
                  ...
                  <div className="space-y-2">
                    <input type="text" placeholder="Uzaklaştırma sebebi" value={banReasonInput} onChange={(e) => setBanReasonInput(e.target.value)} className="w-full bg-black border border-white/10 rounded-xl px-3 py-2 text-xs text-white" />
                    <button type="button" onClick={async () => {
                      if (!banReasonInput) { alert('Sebep yazın.'); return }
                      await supabase.from('profiles').update({ ban_reason: banReasonInput }).eq('id', selectedMember.id)
                      alert('Üye uzaklaştırıldı!')
                      setSelectedMember(null); setBanReasonInput(''); fetchData()
                    }} className="px-4 py-2 bg-red-600 text-white rounded-xl text-xs font-bold cursor-pointer">Uzaklaştır</button>
                  </div>
                )}
              </div>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/85 p-4 backdrop-blur-sm" onClick={() => setEditingEvent(null)}>
          <div className="w-full max-w-lg rounded-3xl border border-white/20 bg-zinc-950 p-6 sm:p-8 shadow-2xl space-y-6 max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleUpdateEvent} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <input type="text" value={editEvtInstructor} onChange={(e) => setEditEvtInstructor(e.target.value)} placeholder="Eğitmen" className="w-full rounded-xl border border-white/10 bg-black px-4 py-3 text-xs text-white" />
