  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-black text-white p-6 sm:p-10 font-sans">
      ...
      <div className="mx-auto max-w-7xl space-y-12 mb-16">
          ...
          <div className="space-y-12">
            ...
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                <h2 className="text-base font-bold flex items-center gap-2"><Users className="h-4 w-4 text-primary" /><span>Etkinlik Katılımcıları ({filteredRegistrations.length})</span></h2>
        ...
        {/* SÜPER ADMIN: ÜYELER VE YETKİ MATRİSİ */}
        {isSuperAdmin && (
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              <h2 className="text-base font-bold flex items-center gap-2 text-red-400"><ShieldAlert className="h-5 w-5" /><span>Süper Admin: Tüm Üyeler ({profiles.length})</span></h2>
            ...
            <div className="rounded-3xl border border-white/10 bg-zinc-950 overflow-hidden shadow-xl">
              <div className="overflow-x-auto">
                <table className="w-full text-left text-xs font-mono">
                  <thead className="border-b border-white/10 bg-black/60 text-zinc-500 uppercase">
                    <tr><th className="p-4">Üye</th><th className="p-4">Rol / Branş</th><th className="p-4">Yetki Ata</th></tr>
                  </thead>
                  <tbody className="divide-y divide-white/5 text-zinc-300">
                    {profiles.map((prof) => (
                      <tr key={prof.id} className="hover:bg-zinc-900/40">
                        <td className="p-4">
                          <div className="font-bold text-white flex items-center gap-2">
                            ...
                            {prof.ban_reason && <span className="bg-red-500/20 text-red-400 px-2 py-0.5 rounded text-[9px]">Yasaklı</span>}
                          </div>
                        ...
                        <td className="p-4">
                          <div className="flex flex-wrap gap-2 items-center">
                            ...
                            <button type="button" onClick={() => setSelectedMember(prof)} className="px-3 py-1.5 bg-zinc-800 text-zinc-200 rounded-lg text-[11px] font-bold cursor-pointer inline-flex items-center gap-1"><Eye size={12} /> Detay</button>
                          </div>
                        </td>
                      </tr>
                    ))}
      ...
      {/* Ürün Düzenleme Modal (Fotoğraf Ekleme / Kaldırma & Stok Güncelleme) */}
      {editingProduct && (
          ...
          <div className="relative w-full max-w-lg rounded-3xl border border-white/20 bg-zinc-950 p-6 sm:p-8 shadow-2xl space-y-6 max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleUpdateProduct} className="space-y-4">
              ...
              <div>
                ...
                {editImages.length > 0 && (
                  <div className="flex items-center gap-3 pt-3 overflow-x-auto pb-2">
              ...
              <textarea rows={3} value={editDescription} onChange={(e) => setEditDescription(e.target.value)} placeholder="Ürün Açıklaması (HTML destekler)" className="w-full rounded-xl border border-white/10 bg-black px-4 py-3 text-xs text-white resize-none" />
      ...
      {selectedMember && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/85 p-4 backdrop-blur-md" onClick={() => setSelectedMember(null)}>
          <div className="relative w-full max-w-lg rounded-3xl border border-white/20 bg-zinc-950 p-6 sm:p-8 shadow-2xl space-y-6 max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between border-b border-white/10 pb-4">
              <h3 className="font-bold text-base text-white">Üye Bilgileri</h3>
            ...
            <div className="space-y-4 text-xs font-mono">
              <div className="bg-black/60 p-4 rounded-2xl border border-white/10 space-y-2">
                <div><strong className="text-zinc-500">Ad Soyad:</strong> <span className="text-white font-bold">{selectedMember.full_name}</span></div>
