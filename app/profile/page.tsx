...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    const checkAuth = async () => {
      ...
      if (!session) {
        router.push('/') // Giriş yapmamışsa ana sayfaya gönder
      } else {
  ...
  const loadUserProfileAndEvents = async () => {
    ...
    try {
      ...
      // Profil bilgilerini çek (profiles tablosunu kullanıyoruz)
  ...
  const handlePayTRRedirect = (eventTitle: string, price: number) => {
    alert(`"${eventTitle}" etkinliği için ₺${price} tutarındaki ödeme ekranına yönlendiriliyorsunuz.`)
  }
    ...
    <div className="min-h-screen bg-black text-white font-sans selection:bg-primary selection:text-black p-6 sm:p-10">
      <div className="max-w-5xl mx-auto space-y-10">
        ...
        <div className="flex items-center justify-between border-b border-white/10 pb-6">
          <Link href="/community" className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-zinc-900 px-4 py-2 text-xs font-bold text-zinc-300 hover:text-white transition-colors cursor-pointer">
              ...
              {myRegistrations.map((reg) => {
                ...
                return (
                  <div key={reg.id} className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 rounded-2xl border border-white/10 bg-zinc-950 p-5 shadow-md">
                    ...
                    {reg.is_paid ? null : (
                      <button onClick={() => handlePayTRRedirect(evt.title, evt.price)} className="rounded-full bg-primary px-6 py-3 text-xs font-bold uppercase text-black hover:scale-105 transition-transform cursor-pointer">
                        ₺{evt.price} Ödemeyi Tamamla
                      </button>
                    )}
                  </div>
                )
