  ...
  return (
    <>
      <header className={cn('fixed inset-x-0 top-0 z-50 transition-all duration-300', scrolled ? 'border-b border-white/10 bg-black/90 backdrop-blur-2xl shadow-lg' : 'border-b border-transparent bg-black/50 backdrop-blur-md')}>
        <div className="mx-auto flex h-16 sm:h-20 max-w-7xl items-center justify-between px-4 sm:px-8 lg:px-12">
          ...
          <div className="flex items-center">
            <button 
              ...
              className="sm:hidden flex flex-col justify-center items-center h-9 w-9 rounded-full border border-white/15 bg-zinc-900/80 text-white hover:border-primary transition-all cursor-pointer"
          ...
          {/* Orta: Logo (Telefonda küçültüldü, masaüstünde normal) */}
          <div className="absolute left-1/2 -translate-x-1/2 scale-75 sm:scale-100 origin-center">
            <Link href="/store" aria-label="ORISE STORE" className="flex flex-col items-center">
              <Logo />
            </Link>
          </div>

          {/* Sağ: Sepet ve Giriş (Telefonda sıkışmayı önlemek için daraltıldı) */}
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            {user ? (
              <div className="flex items-center gap-1.5 sm:gap-2">
                <Link href="/profile" className="flex items-center gap-1.5 rounded-full border border-white/15 bg-black/80 px-3 sm:px-4 py-1.5 sm:py-2 text-[11px] sm:text-xs font-mono text-zinc-300 backdrop-blur-md hover:border-primary hover:text-white transition-all cursor-pointer">
                  {avatarUrl ? (
                  ...
                  <span className="truncate max-w-[70px] sm:max-w-[120px]">{fullName || user.email}</span>
                </Link>
                <button onClick={handleLogout} className="flex items-center justify-center h-9 w-9 sm:h-10 sm:w-10 rounded-full border border-red-500/30 bg-red-500/10 text-red-400 hover:bg-red-500/20 transition-all cursor-pointer shrink-0" title="Çıkış Yap">
                  <LogOut className="h-3.5 w-3.5 sm:h-4 sm:w-4" />
                </button>
              </div>
            ) : (
              <button type="button" onClick={() => setIsAuthOpen(true)} className="flex items-center gap-1.5 sm:gap-2 rounded-full border border-primary/40 bg-primary/10 px-3 sm:px-5 py-1.5 sm:py-2.5 text-[11px] sm:text-xs font-bold uppercase tracking-wider text-primary backdrop-blur-md hover:bg-primary/25 hover:border-primary transition-all cursor-pointer shrink-0">
        ...
        {/* Mobil Açılır Menü */}
        {mobileMenuOpen && (
          <div className="sm:hidden absolute top-16 inset-x-0 bg-zinc-950/95 border-b border-white/10 backdrop-blur-2xl p-6 space-y-4 font-mono text-xs uppercase font-bold animate-fadeIn">
