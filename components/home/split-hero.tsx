...
export function SplitHero() {
  ...
  return (
    <section className="relative flex h-full w-full flex-col overflow-hidden md:flex-row bg-black">
      {/* Merkez Reaktif Reaktör Logo */}
      <div className="pointer-events-none absolute left-1/2 top-1/2 z-30 hidden -translate-x-1/2 -translate-y-1/2 md:block">
        {/* Arkadaki Glow */}
        <div
          className={cn(
            'absolute inset-0 -m-8 rounded-full bg-primary/25 blur-3xl transition-all duration-700',
            hovered ? 'scale-150 opacity-100 bg-primary/45' : 'scale-100 opacity-50',
          )}
        />

        <div
          className={cn(
            'relative flex h-20 w-20 items-center justify-center rounded-full border bg-background/90 backdrop-blur-2xl transition-all duration-500 lg:h-24 lg:w-24 shadow-[0_0_50px_rgba(0,0,0,0.8)]',
            hovered === 'community'
              ? 'border-primary -translate-x-2 shadow-[-25px_0_60px_rgba(249,115,22,0.5)]'
              : hovered === 'store'
                ? 'border-primary translate-x-2 shadow-[25px_0_60px_rgba(249,115,22,0.5)]'
      ...
      {PANELS.map((panel) => {
        ...
        return (
          <Link
            ...
            className={cn(
              'group relative flex flex-col justify-center border-b border-border/30 p-8 transition-all duration-700 ease-out last:border-b-0 md:h-full md:border-b-0 md:border-r md:last:border-r-0',
          ...
          >
            ...
            {/* Yan Kenar Editoryal Damgası (Lüks Marka Detayı) */}
            <div
              className={cn(
                'pointer-events-none absolute top-1/2 -translate-y-1/2 hidden xl:block text-[10px] font-mono tracking-[0.3em] text-muted-foreground/30 uppercase [writing-mode:vertical-rl] transition-colors duration-500 group-hover:text-primary/60',
